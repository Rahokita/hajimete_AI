...
def generates(chr, listdata):
    """文の生成"""
    # 出力文字列保存用変数nextlineの初期化
    nextline = ""
    # 開始文字の出力
    print(chr, end = '')
    nextline= nextline + chr # 文字の保存
    # 続きの出力
    while True:
        # 次の文字の決定
...
# 会話しましょう
print("さくら：メッセージをどうぞ")
inputline = "人工無能を人工人格に昇華させよう。"
for i in range(20) :  # 会話しましょう          
    # 形態素の2-gramデータの生成
    listdata = []
    make2gram(inputtext, listdata)
    # ユーザからの入力の代わりに、inputlineを解析
    inputlist = []
    # カタカナか漢字からなる形態素の抽出
    ...
    # メッセージの作成      
    print("さくら： ", end = '')
    nextline = generates(startch, listdata)
    # 入力が句点で終わっていなければ追加する
    if not re.match('[。．]', inputline[-1] ):
        inputline = inputline + '。'
    # 生成データinputtextの更新
    inputtext = inputtext + inputline 
    inputline = copy.copy(nextline) # 自問自答   
print("さくら：ばいば～い") 
